...
const DailyNumbers = ({ countryCode }) => {
  ...
  return (
    <Card className="card card-default card-demo historical-card">
      <Card.Header as="h5">
        Cases and deaths per day in <strong>{countryCode}</strong>
      </Card.Header>
      <Card.Body>
        <Card.Text>
          <ButtonGroup
          ...
          >
            {({ fetching, failed, data }) => {
              ...
              if (data && data.timeline) {
                ...
                console.log("logz", data);
                console.log(
                  "logza",
                  keys.map(k => data.timeline.cases[keys[previousIndex(k)]])
                );
                  ...
                  datasets: [
                    {
                      ...
                      data: keys.map(k => ({
                        x: new Date(k),
                        y:
                          data.timeline.cases[k] -
                          data.timeline.cases[keys[previousIndex(k)]]
                      })),
                      borderColor: "#4271b3",
                    ...
                    },
                    {
                      ...
                      data: keys.map(k => ({
                        x: new Date(k),
                        y:
                          Number(data.timeline.deaths[k]) -
                          Number(data.timeline.deaths[keys[previousIndex(k)]])
                      })),
                      borderColor: "#ff3030",
                ...
                console.log("logz", chartData.datasets);
